...
from src.models.cluster_garnoldi import FILTER_PROFILES
...
class TwoStageTrainer:
    ...
    def __init__(
        ...
        split_seed: int = 42,
        optuna_seed: int = 42,
        s1_trial_max_epochs: int = 200,
        s2_trial_max_epochs: int = 200,
        s1_retrain_max_epochs: int = 500,
        s2_retrain_max_epochs: int = 2000,
        s1_trial_patience: int = 50,
        s2_trial_patience: int = 100,
        s1_retrain_patience: int = 100,
        s2_retrain_patience: int = 100,
    ):
        self.accelerator = accelerator
        self.split_index = split_index
        self.split_seed = split_seed
        self.optuna_seed = optuna_seed
        self.s1_trial_max_epochs = s1_trial_max_epochs
        self.s2_trial_max_epochs = s2_trial_max_epochs
        self.s1_retrain_max_epochs = s1_retrain_max_epochs
        self.s2_retrain_max_epochs = s2_retrain_max_epochs
        self.s1_trial_patience = s1_trial_patience
        self.s2_trial_patience = s2_trial_patience
        self.s1_retrain_patience = s1_retrain_patience
        self.s2_retrain_patience = s2_retrain_patience
    ...
    def _stage1_objective(self, trial, network_name, cut_type):
        ...
        early_stop = EarlyStopping(
            monitor="val_loss",
            patience=self.s1_trial_patience,
            verbose=False,
            mode="min",
        )
        ...
        trainer = pl.Trainer(
            max_epochs=self.s1_trial_max_epochs,
    ...
    def _train_stage1_best(self, network_name, cut_type, best_params):
        ...
        early_stop = EarlyStopping(
            monitor="val_loss",
            patience=self.s1_retrain_patience,
            verbose=False,
            mode="min",
        )
        ...
        trainer = pl.Trainer(
            max_epochs=self.s1_retrain_max_epochs,
    ...
    def _stage2_objective(self, trial, network_name, pretrained_clusters, num_clusters):
        ...
        filter_profile = trial.suggest_categorical(
            "s2_filter_profile", sorted(FILTER_PROFILES.keys())
        )
        ...
        early_stop = EarlyStopping(
            monitor="val_loss",
            patience=self.s2_trial_patience,
            verbose=False,
            mode="min",
        )
        pruning = _OptunaPruning(trial, monitor="val_loss")
        ...
        trainer = pl.Trainer(
            max_epochs=self.s2_trial_max_epochs,
            callbacks=[early_stop, pruning],
            logger=logger,
            enable_progress_bar=False,
            enable_model_summary=False,
    ...
    def run_optimization(
        ...
        study_s2 = optuna.create_study(
            direction="minimize",
            pruner=pruner2,
            sampler=optuna.samplers.TPESampler(seed=self.optuna_seed + 1),
        )
        study_s2.optimize(
    ...
    def test_best_model(
        ...
        early_stop = EarlyStopping(
            monitor="val_loss",
            patience=self.s2_retrain_patience,
            verbose=False,
            mode="min",
        )
        ...
        trainer = pl.Trainer(
            max_epochs=self.s2_retrain_max_epochs,
            callbacks=[early_stop],
            logger=logger,
            log_every_n_steps=1,
            accelerator=self.accelerator,
        ...
        filter_coefficients = model.get_filter_coefficients()
        print("\nLearned filter coefficients:")
        for name, coeffs in filter_coefficients.items():
            print(f"  {name}: {[f'{c:.4f}' for c in coeffs]}")

        # Cluster statistics from Stage-1 logits
        with torch.no_grad():
            q = torch.softmax(pretrained_clusters.float(), dim=-1)
            hard_assignments = q.argmax(dim=-1)
            hard_counts = torch.bincount(hard_assignments, minlength=num_clusters)

            p = q.clamp_min(1e-12)
            entropy = float((-p * p.log()).sum(dim=-1).mean().item())
            normalized_entropy = entropy / max(math.log(num_clusters), 1e-12)

            cluster_stats = {
                "num_nodes": int(q.shape[0]),
                "num_clusters": int(num_clusters),
                "mean_membership": [float(v) for v in q.mean(dim=0).tolist()],
                "hard_counts": [int(v) for v in hard_counts.tolist()],
                "hard_fractions": [
                    float(v) for v in (hard_counts.float() / max(int(q.shape[0]), 1)).tolist()
                ],
