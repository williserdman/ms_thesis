...
# Small curated filter-profile pool for conservative search.
FILTER_PROFILES: dict[str, list[str]] = {
    "diverse": [
        "g_low_pass",
        "g_high_pass",
        "g_band_pass",
...
def _laplacian_trace_from_edges(
    ...
    """Compute tr(Q^T L Q) from Laplacian COO edges without sparse matmul.

    This avoids SparseMPS backend limitations around sparse COO tensor ops.
    """
...
class ClusterStage1(nn.Module):
    ...
    def forward(self, data):
        ...
        edge_index_lap, edge_weight_lap = get_laplacian(
            cast(torch.Tensor, edge_index_norm), edge_weight, num_nodes=N
        )
        edge_weight_lap = cast(torch.Tensor, edge_weight_lap)
        cut_value = _laplacian_trace_from_edges(edge_index_lap, edge_weight_lap, Q)
...
class ClusterGArnoldiFilterBank(nn.Module):
    ...
    def __init__(
        ...
        if filter_profile not in FILTER_PROFILES:
            valid = ", ".join(sorted(FILTER_PROFILES.keys()))
            raise ValueError(
                f"Unknown filter_profile '{filter_profile}'. Expected one of: {valid}"
            )
        self.selected_filter_names = FILTER_PROFILES[filter_profile]
        ...
        for c in range(num_clusters):
            fname = self.selected_filter_names[c % len(self.selected_filter_names)]
    ...
    def get_filter_coefficients(self) -> dict[str, list[float]]:
        ...
        for c, filt in enumerate(self.filter_bank):
            label = self.selected_filter_names[c % len(self.selected_filter_names)]
            coeffs = filt.temp.detach().cpu().view(-1)  # type: ignore[misc]
            out[f"cluster_{c} ({label})"] = [float(v) for v in coeffs]
        return out
    ...
    def forward(self, data):
        ...
        Q = F.softmax(cast(torch.Tensor, self.cluster_logits), dim=-1)  # (N, num_clusters)
