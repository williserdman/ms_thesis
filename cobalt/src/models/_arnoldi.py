...
def j_polynomial_zeros(nt, alpha, beta):
    ...
    zemu = (np.power(2.0, (ab + 1.0)) * gamma(alpha + 1.0) * gamma(beta + 1.0)) / gamma(
        abi
    )
...
def t_polynomial_zeros(x0, x1, n):
    return (x1 - x0) * (
        np.cos((2 * np.arange(1, n + 1) - 1) / (2 * n) * np.pi) + 1
    ) / 2 + x0
        ...
        **kwargs,
        ...
        if nameFunc == "g_0":
            self.coeffs = compare_fit_panelA(
                g_0, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_1":
            self.coeffs = compare_fit_panelA(
                g_1, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_2":
            self.coeffs = compare_fit_panelA(
                g_2, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_3":
            self.coeffs = compare_fit_panelA(
                g_3, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_4":
            self.coeffs = compare_fit_panelA(
                g_4, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_band_rejection":
            self.coeffs = compare_fit_panelA(
                g_band_rejection, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_band_pass":
            self.coeffs = compare_fit_panelA(
                g_band_pass, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_low_pass":
            self.coeffs = compare_fit_panelA(
                g_low_pass, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_high_pass":
            self.coeffs = compare_fit_panelA(
                g_high_pass, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        elif nameFunc == "g_comb":
            self.coeffs = compare_fit_panelA(
                g_comb, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
        else:
            self.coeffs = compare_fit_panelA(
                g_fullRWR, namePoly, Vandermonde, self.K, self.lower, self.upper
            )
