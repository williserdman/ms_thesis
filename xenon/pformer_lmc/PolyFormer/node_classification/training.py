...
from mymodels import PolyFormer, GCN
import torch
...
def RunExp(args, dataset, data, Net):
    ...
    if not getattr(args, 'use_external_splits', False):
    ...
    model_tag = args.net.lower()
    ckpt_prefix = args.base if model_tag == "polyformer" else model_tag

    if args.save_models:
        dataset_dir = os.path.join(args.model_dir, args.dataset.lower())
        os.makedirs(dataset_dir, exist_ok=True)
        checkpoint_path = os.path.join(
            dataset_dir,
            f"{ckpt_prefix}_run{args.idx_run + 1}_seed{args.seed}.pt",
        )
    ...
    for epoch in range(args.epochs):
                ...
                torch.save(
                    {
                        ...
                        "test_acc_at_best_val": float(test_acc),
                    },
                    checkpoint_path,
                )
        else:
        ...
        if bad_counter == args.early_stopping:
            break
    return test_acc, best_val_acc, checkpoint_path
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, choices=['cora', 'citeseer', 'pubmed', 'computers', 'photo', 'chameleon_filtered', 'squirrel_filtered', 'actor', 'texas', 'cornell', 'roman-empire', 'amazon-ratings', 'minesweeper', 'tolokers', 'questions'], default='tolokers')
    parser.add_argument('--device_idx', type=int, default=0, help='GPU device.')
    parser.add_argument('--net', type=str, choices=['PolyFormer', 'GCN'], default='PolyFormer', help='network name.')
    parser.add_argument('--runs', type=int, default=10, help='runs.')
    parser.add_argument('--model_dir', type=str, default='./saved_models', help='directory to store model checkpoints.')
    parser.add_argument('--no_save_models', action='store_true', help='disable saving checkpoints during training.')
    ...
    print("---------------------------------------------")
    gnn_name = args.net
    Net = PolyFormer if args.net == 'PolyFormer' else GCN

    results = []
    for RP in range(args.runs):
        args.idx_run = RP
        print("RP", RP, "Launched...")
        args.seed=SEEDS[RP % len(SEEDS)]
        dataset, data = get_data_load(args, split_index=RP, split_seed=args.seed)
        if RP == 0:
            print("args.num_classes", args.num_classes)
        test_acc, best_val_acc, checkpoint_path = RunExp(args, dataset, data, Net)
