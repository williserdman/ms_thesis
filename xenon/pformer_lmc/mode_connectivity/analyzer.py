...
class ModeConnectAnalyzer:
    ...
    def all_close_test(self) -> bool:
        return torch.allclose(self._flatten_params(self.theta_a), self._flatten_params(self.theta_b))

    def euclidean_distance(self) -> float:
        return float(torch.linalg.norm(self._flatten_params(self.theta_a) - self._flatten_params(self.theta_b)))
