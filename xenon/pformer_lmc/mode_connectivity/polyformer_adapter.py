...
from mymodels import PolyFormer  # noqa: E402
...
def build_model_args(dataset: str, base: str = "mono", device_idx: int = 0) -> argparse.Namespace:
    ds = DATASET_ALIASES.get(dataset.lower(), dataset.lower())
    if ds not in DATASET_HPARAMS:
        raise ValueError(f"Unsupported dataset '{dataset}'. Supported: {sorted(DATASET_HPARAMS.keys())}")
    hparams = DATASET_HPARAMS[ds]
...
def build_polyformer_manager(
    ...
    args = build_model_args(dataset_name, base=base, device_idx=0)
    args.seed = run_seed
    ...
    model = PolyFormer(dataset, args).to(device)
    data = data.to(device)
