...
class ClusteredArnoldiModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        lap = torch.sparse_coo_tensor(
            edge_index_lap, edge_weight_lap, (N, N), device=x.device, dtype=x.dtype
        ).coalesce()

        Lc = torch.sparse.mm(lap, clusters)  # L @ Q
        qtLq = clusters.t() @ Lc  # Q^T @ L @ Q
        cut_size = torch.trace(qtLq)
